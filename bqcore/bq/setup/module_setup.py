...
def needs_update(output, dependency):
    ...
    """
    return not os.path.exists(output) or (os.path.exists(
        dependency) and os.path.getmtime(dependency) > os.path.getmtime(output))
...
def ensure_matlab(params):
    ...
    if matlab_home not in os.environ['PATH']:
        os.environ['PATH'] = os.path.join(
            matlab_home, 'bin') + os.pathsep + os.environ['PATH']
...
def matlab_setup(main_path, files=[], bisque_deps=False,
                 dependency_dir="mbuild", params={}, **kw):
    ...
    if not (needs_update(ctf, source) or needs_update(main, source)
        ...
        print(
            "Skipping matlab compilation %s is newer than %s" %
            (ctf, source))
        return 0

    if mcc(source, '-d', dependency_dir, '-m', '-C',
           '-R', '-nodisplay', '-R', '-nosplash', *rest):
...
def python_setup(scripts, package_scripts=True,
                 dependency_dir='pydist', params=None):
    ...
    filename = find_config_path(
        'templates/python_launcher.tmpl', config_dir='config-defaults').replace('\\', '/')
    print("TEMPLATE", filename)
    for script in scripts:
        script_name = os.path.splitext(script)[0]
        data['script'] = os.path.join(
            '.', dependency_dir, script_name, script_name)
...
def docker_setup(image, command, base_image=None, params=None):
    ...
    if not os.path.exists('Dockerfile'):
        ...
        with open('Dockerfile', 'w') as f:
            maintainer = docker_params.get(
                'docker.maintainer', 'nobody@example.com')
            # check for configured base i.e docker.image.matlab_runtime
            base = docker_params.get('docker.image.%s' %
    ...
    if docker_hub:
        ...
        if docker_user and docker_pass:
            check_call(['docker', 'login', '-u', docker_user, '-p',
                        docker_pass, '-e', docker_email, docker_hub])
